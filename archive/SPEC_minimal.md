...
This is a **security-first profile** of the Symphony Service Specification.
See `SPEC_alignment.md` for what's preserved and what diverges.
...
validation commands, or approval list during a run. Deliberate divergence
from Symphony's "self-contained WORKFLOW.md" — see `SPEC_alignment.md` A1.
...
claude:
  ...
  implementation_tools:
    ...
    - "Bash(git diff:*)"
    - "Bash(go test:*)"
  review_tools:          ["Read", "Grep", "Glob", "LS"]   # reviewer phase
  disallowed_tools:
    - "Bash(sudo:*)"
    - "Bash(curl:*)"
    - "Bash(wget:*)"
...
block. Without it, `auto` mode falls through to `gated` (human approval).
...
internal/
  ...
  runner/        # AgentRunner interface + claude.go + codex.go
  approval/      # rules engine + reviewer driver + diff verification
...
auto-approved issues skip directly from `planning` to `implementing`. The
state-label transition is still atomic via `ReplaceStateLabel(remove, add)`.
...
After lock, before dispatch. For every open issue with any `symphony:*`
label and every file under `state/jobs/`, apply exactly one row from the
reconcile table. The full table lives in `SPEC_PATCH.md` (Patch 4).
Load-bearing rows:

- Local `implementing` + GitHub `implementing` → blocked. Never auto-resume.
- Local `implementing` + GitHub `pr-ready` → search PRs by branch, save
  `pr_number`, finalize.
- Local missing + GitHub active label → blocked, "orphan label" comment.
- Issue closed → mark local complete, kill running job.

Reconciliation never starts an agent. Never deletes a workspace.
...
Edit/Write/Bash at the tool level (not via prompt). Planning and review use
`plan`. Implementation uses `acceptEdits`. Never `bypassPermissions`.
...
Symphony default. Use only when the workspace + tool restrictions are
sufficient for your safety model.
...
3. If `len(extra) > max_diff_drift_files`: set `blocked`, post a comment
   listing `extra`, do not commit. The agent claimed a smaller scope than
   it produced — auto-approval is invalidated post-hoc.
...
scope is small enough to clear rules, (b) the reviewer to approve,
(c) the actual diff to match the claimed scope. The combination is hard
to break with prompt injection alone.
...
12. If `approval.mode == "auto"`:
    - `auto.rules` has at least one entry with `issue_labels: []` OR
      `fallback_on_no_rule_match` is set. Otherwise warn (issues without
      matching rules silently fall through).
    - `auto.reviewer.provider` is in PATH (if any rule has
      `reviewer_required: true`).
    - Warn if `auto.reviewer.provider == agent.provider` (defeats the
...
- auto diff drift exceeds → blocked even though approval path was clean
...
## 16. Symphony alignment summary

`SPEC_alignment.md` carries the full audit. In one paragraph: this is a
security-first profile of Symphony Service Specification Draft v1, with an
optional `auto` mode that uses rules + reviewer + diff verification to
reduce human friction without giving up safety.

**Preserved:** per-issue workspace isolation, in-repo `WORKFLOW.md` prompt,
workspace hooks, bounded concurrency, reconciliation before dispatch,
restart recovery without DB, both Codex backends, JSONL agent protocol
contract, sanitized workspace key, hard cwd validation, stop semantics.

**Diverged (security-first):** config split out of repo (A1), orchestrator
owns PR push, structured plan-output contract, post-impl diff verification.

**Diverged (inherited from prior SPEC):** GitHub Issues instead of Linear,
no `linear_graphql` tool, no agent-controlled tracker writes.

**New:** `approval.mode: auto` adds a rules engine + reviewer agent (D + C
combined) with prompt-injection-immune diff verification. Closer to
Symphony's no-human-gate default while keeping orchestrator-owned PR push
as the final safety boundary.

**Deferred:** multi-turn continuation, per-state concurrency limits,
blocker-aware dispatch, event-inactivity stall detection (lights up under
`codex.mode = app-server`), dynamic config reload during runs.
...
agent-controlled tracker writes, naive single-agent self-approve (provides
no safety).
